...
def detect_environment():
    ...
    in_github_env = os.environ.get("GITHUB_ACTIONS") == "true" or (
...
if __name__ == "__main__":
    ...
    # 先收集账号密码配置
    user = os.getenv("DEEPFLOOD_USER")
    password = os.getenv("DEEPFLOOD_PASS")
    if user and password:
        accounts.append({"user": user, "password": password})

    index = 1
    while True:
        user = os.getenv(f"DEEPFLOOD_USER{index}")
        password = os.getenv(f"DEEPFLOOD_PASS{index}")
        if user and password:
            accounts.append({"user": user, "password": password})
            index += 1
        else:
    ...
    if cookies_updated and cookie_list:
        ...
        try:
            save_cookie("DEEPFLOOD_COOKIE", all_cookies_new)
            print("所有Cookie已成功保存")
        except Exception as e:
